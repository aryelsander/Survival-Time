...
func setup(spawn_position : Vector2,speed : float,target : Node2D,damage : float,enemy : UnitType) -> void:
	super(spawn_position,speed,target,damage,enemy)
	_scale = scale
	pulse_effect()
	
func _physics_process(_delta: float) -> void:
	_direction = position.direction_to(target_enemy.position)
